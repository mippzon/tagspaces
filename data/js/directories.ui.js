...
define((require, exports, module) => {
  ...
  let homeFolderTitle = 'Home';
  let directoryHistory = [];
  const metaTagGroupsHistory = null;
  const dir4ContextMenu = null;
  const folderPropertiesOpened = false;
  ...
  class TSDirectories {
    ...
    static loadFolderMetaData(path, element) {
      let historyItem = getDirHistoryItem(path);
    ...
    static loadMetaTagGroups(metaData) {
    ...
    static updateSubDirs(dirList) {
      ...
      this._generateAlternativeDirPath();
      this._handleDirCollapsion();
    }

    static generateAlternativeDirPath() {
      ...
      $alternativeNavigator.find('.createSubdirectory').on('click', () => {
        showCreateDirectoryDialog($(this).attr('data-path'));
      });

      $alternativeNavigator.find('.renameDirectory').on('click', () => {
        showRenameDirectoryDialog($(this).attr('data-path'));
      });

      $alternativeNavigator.find('.dropdown-toggle').on('contextmenu click', () => {
        TSCORE.hideAllDropDownMenus();
        $('#dirMenu' + $(this).attr('data-menu')).css("display", "block");
        return false;
      });

      $alternativeNavigator.find('.close').on("click", () => {
        TSCORE.hideAllDropDownMenus();
      });

      $alternativeNavigator.find('.dirButton').on("click", () => {
        navigateToDirectory($(this).attr('data-path'));
      });
    ...
    static getDirectoryCollapsed(directoryPath) {
      for (let i = 0; i < directoryHistory.length; i++) {
    ...
    static setDirectoryCollapse(directoryPath, collapsed) {
      for (let i = 0; i < directoryHistory.length; i++) {
        if (directoryHistory[i].path === directoryPath) {
      ...
      TSCORE.Meta.loadFolderMetaDataPromise(TSCORE.currentPath).then((metaData) => {
        ...
        $('#folderTagsProperty').select2({
          ...
          formatSelectionCssClass: function(tag, container) {
            let style = TSCORE.generateTagStyle(TSCORE.Config.findTag(tag.text));
            if (style) {
    ...
    static cancelEditFolderDescription() {
    ...
    static createLocation() {
      let locationPath = $('#folderLocation').val();
    ...
    static selectLocalDirectory() {
      TSCORE.IO.selectDirectory();
    }

    static showLocationEditDialog(name, path) {
      require(['text!templates/LocationEditDialog.html'], (uiTPL) => {
        ...
        let selectedPerspectiveId = TSCORE.Config.getLocation(path).perspective;
    ...
    static showLocationCreateDialog() {
      require(['text!templates/LocationCreateDialog.html'], (uiTPL) => {
        ...
        if ($dialogCreateFolderConnection.length < 1) {
          ...
          if (isWeb) {
            ...
            $('#selectLocalDirectory').on('click', (e) => {
              e.preventDefault();
              selectLocalDirectory();
            });
          }
    ...
    static showRenameDirectoryDialog(dirPath) {
  ...
  exports.TSDirectories = TSDirectories;
